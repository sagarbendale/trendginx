...
public class TweetRateCurrentHour {
	...
	public void getRecords(int req_id)
	{
		...
		try
		{
			topStatement = dbConnection.prepareStatement("SELECT COUNT(*) FROM trendginx.twitter_result where req_id="+req_id+" AND twt_timestamp Between DATE_SUB(NOW(), INTERVAL 1 hour) AND NOW()");
