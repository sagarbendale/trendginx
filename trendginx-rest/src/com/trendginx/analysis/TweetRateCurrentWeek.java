...
public class TweetRateCurrentWeek {
	...
	public void getRecords(int req_id)
	{
		...
		try
		{
			topStatement = dbConnection.prepareStatement("SELECT COUNT(*) FROM trendginx.twitter_result where req_id="+req_id+" AND twt_timestamp BETWEEN DATE_SUB(NOW(), INTERVAL 1 week) AND NOW()");
