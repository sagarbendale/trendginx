...
public class TweetRateLastDay
{
	...
	public void getRecords(int req_id)
	{
		...
		try
		{
			topStatement = dbConnection.prepareStatement("select * from trendginx.twitter_result where req_id="+req_id+" AND twt_timestamp BETWEEN (SELECT (DATE_SUB(NOW(), INTERVAL 2 day))) AND (SELECT (DATE_SUB(NOW(), INTERVAL 1 day)))");
